...
// Compound annual growth: FV = PV * (1 + r)^n + PMT * [((1+r)^n - 1) / r]
...
function calculateRiskScore(assets: ForecastInput['assets']): number {
  ...
  for (const asset of assets) {
    ...
    switch (asset.type) {
      case 'INVESTMENT':
        riskFactor = 80 // stocks = high risk
        break
      case 'SAVINGS':
        riskFactor = 10 // deposits = low risk
        break
      case 'PENSION':
        riskFactor = 30 // pension = moderate
        break
      case 'PROVIDENT':
        riskFactor = 35 // provident fund = moderate
        break
      case 'STUDY_FUND':
        riskFactor = 40 // study fund = moderate
        break
      case 'CHECKING':
        riskFactor = 5 // cash = near zero risk
        break
      default:
        riskFactor = 20
    }
...
export function runForecast(input: ForecastInput): ForecastResult {
  ...
  // Project each asset to retirement age
  ...
  for (const asset of assets) {
    ...
    const fv = futureValue(asset.currentValue, rate, yearsToRetirement, monthly)
    projectedRetirementValue += fv
  }

  // Estimate monthly income: 4% withdrawal rule / 12
  const estimatedMonthlyIncome = (projectedRetirementValue * 0.04) / 12

  // Growth projection points (every year)
