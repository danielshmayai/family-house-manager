...
export async function sendOtpEmail(email: string, code: string) {
  const subject = 'קוד הכניסה שלך ל-FamFlow'
  const html = `
    <div style="font-family:system-ui,sans-serif;max-width:480px;margin:0 auto;padding:32px;background:#f9fafb;border-radius:16px;">
      ...
      <h1 style="margin:0 0 8px;font-size:24px;font-weight:800;color:#1f2937;text-align:center;">קוד הכניסה שלך</h1>
      <p style="color:#6b7280;text-align:center;margin:0 0 32px;">הכנס את הקוד הבא כדי להתחבר. הקוד תקף ל-10 דקות.</p>
      <div style="background:white;border-radius:12px;padding:24px;text-align:center;border:2px solid #e5e7eb;margin-bottom:24px;">
        <div style="font-size:40px;font-weight:800;letter-spacing:10px;color:#667eea;font-family:monospace;">${code}</div>
      </div>
