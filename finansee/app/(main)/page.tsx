...
import { TrendChart, AllocationPie } from '@/components/PortfolioChart'
import AssetIcon, { ASSET_COLORS } from '@/components/AssetIcon'
import AddAssetDrawer from '@/components/AddAssetDrawer'
import { generateShortTermProjection } from '@/lib/forecasting'
    ...
    <>
      <div className="fs-page">
        ...
        {topAssets.length > 0 ? (
          <div>
            <div className="fs-section-header">
              ...
              <Link href="/assets" style={{ fontSize: 13, color: 'var(--fs-primary)', textDecoration: 'none' }}>
                הכל ←
              </Link>
            </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 10 }}>
          {[
            { href: '/forecast', icon: '📈', label: 'תחזית' },
            { href: '/goals', icon: '🎯', label: 'יעדים' },
            { href: '/notifications', icon: '🔔', label: 'התראות' },
          ].map(item => (
